...
def atelier():
    ...
    array = oracle.execute_sql(
        'select ROWNUM, (SELECT P.DESCRIPCION FROM AR_PROVINCIAS P WHERE T.CODIGO_PROVINCIA = P.CODIGO_PROVINCIA ), (SELECT C.DESCRIPCION FROM AR_CIUDADES C WHERE T.CODIGO_CIUDAD = C.CODIGO_CIUDAD AND C.CODIGO_PROVINCIA = T.CODIGO_PROVINCIA), T.DESCRIPCION, T.NOMBRE_CONTACTO, T.TELEFONO1, T.TELEFONO2, T.TELEFONO3, T.DIRECCION, T.FECHA_ADICION, T.FECHA_MODIFICACION '
        'from AR_TALLER_SERVICIO_TECNICO T WHERE T.CODIGO_EMPRESA = 20 AND T.ES_TALLER_AUTORIZADO = 1')
    mydict = create_dict()
    for row in array:
        mydict.add(row[0],({"PROVINCIA":row[1],"CIUDAD":row[2],"NOOMBRE TALLER":row[3], "NOMBRE MECANICO":row[4], "NUMERO PRINCIPAL":row[5],"NUMERO ALTERNATIVO":row[6],"NUMERO CONVENCIONAL PRINCIPAL":row[7],"DIRECCION":row[8], "FECHA CREACION":row[9], "FECHA MODIFICACION":row[10]}))
    stud_json = json.dumps(mydict, indent=2, default=str)
    return stud_json
